...
class Slider:
    ...
    def Hurst(df, **kwargs):
        ...
        hurstDF = pd.DataFrame(hurstMat, index=df.columns, columns=["Hurst"])
    ...
    class AI:
        ...
        def gRollingManifold(manifoldIn, df0, st, NumProjections, eigsPC, **kwargs):
            ...
            for i in range(st, len(df0) + 1):
                ...
                if manifoldIn == 'PCA':
                    ...
                    c = 0
                    for eig in eigsPC:
                        #print(eig, ',', X_lle[:, eig].shape) # 0 , 100 , 5
                        #time.sleep(3000)
            ...
            if contractiveObserver == 0:
                ...
                for k in range(len(Comps)):
                    ...
                    if manifoldIn == "PCA":
                        principalCompsDf[k] = pd.concat(
                            [pd.DataFrame(np.zeros((st - 1, len(df.columns))), columns=df.columns),
                             pd.DataFrame(Comps[k], columns=df.columns)], axis=0, ignore_index=True)
                        ...
                        principalCompsDf[k] = pd.concat(
                            [pd.DataFrame(np.zeros((st - 1, pd.DataFrame(Comps[k]).shape[1]))),
                             pd.DataFrame(Comps[k])], axis=0, ignore_index=True)
                ...
                for k in range(len(Comps)):
                    ...
                    principalCompsDf[k] = pd.concat(
                        [pd.DataFrame(np.zeros((st - 1, len(df0.columns))), columns=df.columns),
                         pd.DataFrame(Comps[k], columns=df.columns)], axis=0, ignore_index=True)
                    principalCompsDf[k].index = df0.index
                    principalCompsDf[k] = principalCompsDf[k].fillna(0).replace(0, np.nan).ffill()
                    ...
                    principaleigCoeffsDf[k] = pd.concat(
                        [pd.DataFrame(np.zeros((st - 1, len(df.columns))), columns=df.columns),
                         pd.DataFrame(eigCoeffs[k], columns=df.columns)], axis=0, ignore_index=True)
        ...
        def gGPRegression(dataset_all, params):
            ...
            megaCount = 0
            scoreList = []
            for i in (range(0, X.shape[0], stepper)):#tqdm
                subProcessingHistory_X, subProcessingHistory_y, subProcessingHistory_real_y = X[i:i+params["SubHistoryLength"]], \
                                                                                              y[i:i+params["SubHistoryLength"]], \
                                                                                              real_y[i:i+params["SubHistoryLength"]]
                subIdx = idx[i:i+params["SubHistoryLength"]]
                ...
                if megaCount == 0:
                    ...
                    if params['Kernel'] == '0':
                        ...
                        mainKernel = Matern(length_scale=1, nu=0.5) + WhiteKernel()
                    elif params['Kernel'] == '2':
                        mainKernel = Matern(length_scale=1, nu=1.5)
                    elif params['Kernel'] == '3':
                        mainKernel = Matern(length_scale=1, nu=1.5) + WhiteKernel()
                    elif params['Kernel'] == '4':
                        mainKernel = Matern(length_scale=1, nu=2.5)
                    elif params['Kernel'] == '5':
                        mainKernel = Matern(length_scale=1, nu=2.5) + WhiteKernel()
